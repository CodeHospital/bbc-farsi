...
### Fixed — Admin feeds index: long feed URLs were clipped instead of readable

- `app/views/admin/feeds/_feed.html.erb`: the URL column used Bootstrap's `text-truncate` (forces `white-space: nowrap` + ellipsis) with only a `title` tooltip to see the full value. Replaced with `word-break: break-all` on the cell's `<small>` so the scheme-stripped URL wraps across multiple lines within the column and is fully visible without hovering; the `max-width: 220px` and title tooltip stay as-is.

### Added — Per-feed Telegram autopost channel (round-the-clock, like the fetch schedule)

- **Migration `20260919010000` — NOT RUN (per project rules). Dev/prod need `bin/rails db:migrate` before `/admin/feeds` will load.** Adds `feeds.autopost_telegram_channel_id` (nullable FK to `telegram_channels`, `nil` = don't autopost this feed — the default). `db/schema.rb` updated by hand + `add_foreign_key`, applied to the test DB with `db:test:prepare`.
- `Feed`: `belongs_to :autopost_telegram_channel` (`optional: true`, `inverse_of:`), an `autoposting` scope, `#autopost_channel_label` (channel name or `"Disabled"`), and `#autoposts?` — true only once the chosen channel is both `enabled` **and** has its own `TelegramChannel#autopost` toggle on. That second gate was a deliberate choice (asked and confirmed): it lets a channel's autoposting be paused without editing every feed pointed at it, at the cost of two settings that must agree — same trade-off the existing `TelegramChannel.autopost` scope already encoded. A bogus `autopost_telegram_channel_id` now fails model validation (`inclusion: { in: ->(_){ TelegramChannel.ids } }`) instead of hitting the raw DB foreign-key constraint, mirroring how `fetch_hour` already handles an out-of-range value.
- `TelegramChannel` gained `has_many :autoposting_feeds, ..., dependent: :nullify` (paired with `Feed`'s `inverse_of:`) — **fixes a real bug the FK addition would otherwise have introduced**: without it, deleting a channel still targeted by a feed would raise `ActiveRecord::InvalidForeignKey` (an unhandled 500) instead of just turning that feed's autoposting off. Regression test added.
- `Autoposter` reworked from channel-fan-out to per-feed routing: `post_translation` now queues to `translation.article.feed.autopost_telegram_channel` only (previously fanned out to *every* `TelegramChannel.autopost`-scoped channel) when `feed.autoposts?`; `run_all`'s queueing phase iterates `Feed.autoposting` and joins `Translation` through `article.feed_id` instead of iterating channels. The delivery phase (pacing, `ready?`, `deliver_pending!`) is unchanged — it already worked per-channel regardless of how a post got queued.
- Admin: `/admin/feeds` Edit/New form gains an "Autopost to" `<select>` (Disabled + every channel); the index gains a sortable "Autopost" column with the same inline-select-and-submit pattern as the fetch-hour Schedule column, via a new `PATCH /admin/feeds/:id/autopost_channel` (mirrors the `schedule` action: blank clears, an unknown id leaves it unchanged, turbo-stream replaces just the row). A feed pointed at a disabled/non-autoposting channel shows a small red "channel not enabled/autoposting" flag inline, so a misconfiguration is visible instead of silently doing nothing. The per-row channel option list (`[name, id]` pairs) is computed once per request (`before_action`) rather than once per row.
- **Fixed while implementing this: `config/initializers/sentry.rb` was activating in the test environment.** The user added a real `sentry_dsn` to Rails credentials since the previous Sentry work landed; because the initializer only checked `SentryConfig.enabled?` (not `Rails.env.test?`), `Sentry.init` started running under `bin/rails test` too, and its background telemetry thread made real HTTP calls that WebMock correctly blocks (`Real HTTP connections are disabled`) — spamming every test run and intermittently hanging/timing out the parallelized suite. Added `next if Rails.env.test?`, matching how `config/environments/test.rb` already forces `ActionMailer`'s `:test` delivery method regardless of `RESEND_API_KEY`, and how both poller initializers (`telegram_autopost_scheduler.rb`/`feed_fetch_scheduler.rb`) already skip test. Sentry now never activates under test, no matter what's in credentials.
- README's periodic-work section explains the per-feed channel + two-gate delivery model.
- 473 tests green (16 new: 7 `Feed` model cases for the new association/validation/scope/label/`autoposts?`; 1 `TelegramChannel` destroy-nullifies regression; 5 `Autoposter` cases reworked/added for per-feed routing incl. multi-feed/multi-channel isolation; 9 `Admin::FeedsController` cases for the form field, inline picker, turbo-stream replace, bogus-id rejection, and the in-row warning flag — note some of these land inside the existing feeds-controller test file alongside the earlier schedule-feature tests, so the file's total grew by more than 9). `rubocop`/`zeitwerk:check`/`brakeman` clean.

### Fixed — `ArticleView`: removed dead geo-IP config, fixed a broken `city`/`country` assignment

- Removed unused leftovers that nothing in the codebase referenced: the `CF_COUNTRY_HEADER`/`CF_FRONT_HEADER`/`GENERIC_COUNTRY_HDR` header-name constants, the `GEO_URL`/`GEO_TIMEOUT` constants, and the `geo_secret` class method — none of these were read by `fetch_country_from_service` (which calls `ipwho.is` directly) or by any other file.
- While in there, fixed a real bug in `fetch_country_from_service`: `city = location_data["city"],\n    country = location_data["country"]` (trailing comma) is a multiple-assignment gotcha — Ruby parsed it as `city = [location_data["city"], location_data["country"]]` with `country` set only as a side effect, so every geolocation lookup cached `city_name` as a two-element array's string form instead of the actual city. Split into two plain assignments.
- No schema/migration changes, no test changes needed (existing 52 tests touching `ArticleView` via `news_controller_test.rb`/`admin/articles_controller_test.rb` still pass). `rubocop` clean.

### Added — Report every rescued exception to Sentry (F-12 from plan2.md)

- Added `sentry-ruby`/`sentry-rails` to the Rails app and `sentry-ruby` to the standalone worker's own `worker/Gemfile`. Both are **inert until a DSN is configured** — nothing is sent anywhere out of the box.
- New `SentryConfig` (credentials-first, ENV fallback — same pattern as `MailerConfig`/`Llmarkt`: `sentry_dsn` credential / `SENTRY_DSN` env) and `config/initializers/sentry.rb`, which only calls `Sentry.init` when `SentryConfig.enabled?`. Deferred to `Rails.application.config.to_prepare` — referencing an autoloaded `app/` constant directly in `config/initializers/*.rb` raises `NameError` at boot (same fix already applied in `config/initializers/action_mailer.rb`/`required_env.rb`). The worker (plain Ruby, no Rails credentials) reads `SENTRY_DSN` from its own environment/`.env` directly.
- **Every `rescue` clause in `app/`, `lib/`, `config/initializers/`, and `worker/worker.rb` now calls `Sentry.capture_exception`** alongside whatever it already did (log, retry, fall back, re-raise) — roughly 40 sites across `ArticleView`/`SearchQuery` (swallowed tracking errors), `Task` (six chain/notify/broadcast rescues), `NewsController`'s 404 handler and priority-bump rescue, every `Api::*` controller, `Publisher`, `LlmarktClient`/`LlmarktSubmitter`, `ArticleImageFetcher`, `FeedFetcher` (including the SSRF-guard `ArgumentError` that re-raises), both in-process poller threads, and the worker's HTTP retry loop, task failure handling, status server, and shutdown path. No rescue was left unreported, including ones that catch an expected/handled condition (a 404, a `RecordNotUnique` race) rather than a bug — deliberate, so nothing needs re-auditing later to decide whether it "counts."
- `Sentry.capture_exception` is a documented no-op when `Sentry.init` was never called, so every one of those ~40 call sites is safe with no DSN configured (verified via `bin/rails runner` and in the worker's own bundle context).
- README gained an "Error Tracking (Sentry)" section; `worker/README.md`'s config table and dependency description gained `SENTRY_DSN`; `.env.example` documents the ENV fallback. Not wired into `config/deploy.yml`/`.kamal/secrets`, matching the existing pattern for other credentials-preferred optional config (`RESEND_API_KEY`, `LLMARKT_*`, `TELEGRAM_ADMIN_*` aren't there either) — set `sentry_dsn` via `bin/rails credentials:edit` for production.
- No schema/migration changes. 11 new tests (`SentryConfig` credentials/ENV precedence; and regression coverage proving the wiring actually fires — `NewsController`'s 404 rescue_from, `SearchQuery.track!`/`.table_exists?`, `BbcFeedFetcher`'s swallowed error and re-raised `ArgumentError`, `Publisher#deliver_pending!`'s Telegram-failure path — each asserts `Sentry.capture_exception` is called with the right exception via `Sentry.stub`). 454 tests green, `rubocop`/`zeitwerk:check`/`brakeman` clean (same 2 pre-existing warnings, nothing new).

### Changed — `/admin/feeds`: schedule is editable inline, URL column no longer dominates the row

- The Schedule column is now an inline `<select>` (Disabled + `00:00`–`23:00`) on every row, so a feed's fetch hour can be set without opening the edit form. Changing it auto-submits to a new `PATCH /admin/feeds/:id/schedule` (`Admin::FeedsController#schedule`) that replaces just that row over Turbo Stream — the same pattern the Enable/Disable toggle already uses. A blank value clears the schedule; an out-of-range hour is rejected by the model validation and the row re-renders unchanged (`@feed.reload`) rather than 500ing.
- The URL column is clipped to 220px with `text-truncate` and the `https://` prefix dropped, so long NYT paths no longer stretch the row. The full URL stays in the DOM (so client-side sorting is unaffected) and is shown via the cell's `title` tooltip.
- Fixed a pre-existing bug in the Enable/Disable turbo-stream path: the replaced row rendered without `telegram_posts_counts_by_feed_id`, so the feed's Posted count/percentage silently fell back to 0 until a full page reload. Both turbo-stream actions now pass the counts through. Covered by a regression test asserting the re-rendered row keeps its "100%".
- No schema/migration changes. 443 tests green (6 new: inline set/clear, turbo-stream row replacement, out-of-range rejection, the index row rendering the selector, and the posted-count regression). `rubocop` clean.

### Added — Per-feed hourly fetch schedule (round-the-clock cron), disabled by default

- **Migration `20260919000001` — NOT RUN (per project rules). Dev/prod need `bin/rails db:migrate` before `/admin/feeds` will load.** Adds two nullable columns to `feeds`: `fetch_hour` (integer, `nil` = not scheduled — the default) and `last_scheduled_fetch_at` (datetime, the once-per-hour claim marker). `db/schema.rb` updated by hand and applied to the test DB with `db:test:prepare`.
- `Feed`: new `FETCH_HOURS` (`0..23`), `validates :fetch_hour, inclusion: { in: FETCH_HOURS }, allow_nil: true`, a `scheduled` scope, and `#fetch_schedule_label` (`"08:00"` / `"Disabled"`). Hours are **server-local** — the sweep compares against `Time.now.hour`, so no timezone conversion happens anywhere (chosen over UTC/Tehran deliberately; see README).
- `FeedIngestor.run_scheduled(now = Time.now)` fetches only the *enabled* feeds whose `fetch_hour` matches the current hour, returning `{ feed_count:, new_count: }`. Each feed is claimed first via a conditional `UPDATE ... WHERE last_scheduled_fetch_at IS NULL OR last_scheduled_fetch_at < <hour start>` (the same claim-then-act pattern `Publisher.deliver_pending!` uses), so a feed is fetched at most once per hour slot however many sweeps race. The claim stamps the sweep's own clock, not `Time.current`, so an injected clock stays self-consistent. The per-feed upsert loop shared with `FeedIngestor.run` was extracted into `ingest_feed`.
- Two things drive the sweep, per the same reasoning as Telegram autopost (external cron isn't confirmed configured on prod, and a cron-only feature would silently never run there): new `bin/rails bbc:fetch_scheduled` rake task for an hourly crontab entry, **and** `config/initializers/feed_fetch_scheduler.rb`, a background thread that polls every 5 minutes under `bin/rails server` only (never console/runner/rake/tests). Running both is safe — that's what the claim guard is for.
- Admin: `/admin/feeds` Edit/New form gains a "Scheduled fetch" `<select>` (Disabled + `00:00`–`23:00`); `fetch_hour` added to the permitted params; the index gains a sortable "Schedule" column showing the slot and how long ago the feed was last auto-fetched (unscheduled feeds sort last). `bbc:fetch` and the admin Fetch buttons are unchanged — they still fetch every enabled feed regardless of schedule.
- 436 tests green (13 new: 6 `FeedIngestor.run_scheduled` cases covering hour matching, disabled feeds, article/task ingest, the once-per-hour claim, the next-hour turnover and the `last_scheduled_fetch_at` stamp; 5 `Feed` validation/label/scope cases; 3 admin controller cases covering the form's 25 options, setting an hour and clearing it). `rubocop`/`zeitwerk:check` clean. `bin/rails bbc:fetch_scheduled` smoke-run against the test DB.
...
- Fix: split `Publisher.post_to_channel` (used by all three trigger paths — autopost, the web button, and the bot tap) into two steps. It now only **claims/queues** the translation+channel pair as a `pending` `TelegramPost` row and returns immediately — no Telegram call, no `article.status = "posted"` yet. A new `Publisher.deliver_pending!(channel)` does the actual Telegram API call for the oldest queued post on a channel, and only `Autoposter.run_all` calls it, gated by the existing `Autoposter.ready?` throttle — so it doesn't matter *how* a post got queued, delivery to a given channel is always paced ~25-40 min apart. `Autoposter.run_all`'s sweep now also covers every *enabled* channel (not just autopost ones) for the delivery step, since a human-approved post can be queued on any of them.
- Because delivery is no longer synchronous, *something* has to keep calling `Autoposter.run_all`. Rather than assume the documented `bbc:autopost` cron entry is actually configured on the production server (confirmed with the user: it isn't, or they're not sure) — which would have silently stopped all Telegram posting outright — added a self-contained in-process poller: `config/initializers/telegram_autopost_scheduler.rb` starts a background thread that calls `Autoposter.run_all` every minute, only under `bin/rails server` (never console/runner/rake/tests). No new gem, no migration, consistent with "no in-app job queue for LLM work" (this isn't LLM work). Safe to run alongside an external cron too, if one is ever added: `Publisher.deliver_pending!` now claims a post with a conditional `UPDATE ... WHERE status = 'pending'` (new transient `"sending"` status on `TelegramPost`) before sending, so two concurrent sweeps can't double-deliver the same post.
- UI now distinguishes "queued, not yet sent" from "posted": the web admin's per-channel Post buttons (translation show + article show pages) and the Telegram bot's inline buttons show "⏳ queued/در صف" instead of silently doing nothing on a second tap while a post waits its turn, and "✓ / ✅ ارسال شد" only once actually delivered.
- No schema/migration changes (`TelegramPost.status` was already a plain string column; `"sending"` is just a new allowed value). 417 tests green (12 new: `test/services/publisher_test.rb`, `test/services/autoposter_test.rb`; 1 existing `TelegramAdminNotifierTest` case updated for the new queued-not-instant behavior). `rubocop`/`zeitwerk:check` clean. Verified live: booted a throwaway dev server, confirmed the new scheduler thread starts and runs its poll loop without error against the real dev DB (a no-op there since the one real channel has no autopost/pending backlog, so nothing was actually sent to the live channel during verification).
...
- `Admin::FeedsController#toggle` now responds with a Turbo Stream (new `app/views/admin/feeds/toggle.turbo_stream.erb`) that swaps just the toggled row via `turbo_stream.replace dom_id(@feed)`, falling back to the existing full-page redirect for non-Turbo requests (same `respond_to` pattern already used by `Admin::TasksController#retry`). The feeds `<tr>` markup was extracted into `app/views/admin/feeds/_feed.html.erb` (rendered as `<%= render @feeds %>` from the index) so the same partial backs both the initial table render and the stream update. Feed sorting (`sort_link` on Name/Category/Source/Status) was already implemented from an earlier session — verified still working, unaffected by this change.
- Verified live end-to-end with a disposable admin user + feed in a throwaway copy of the test DB (not dev/prod): clicking Enable/Disable now fires exactly one `POST .../toggle` request returning `text/vnd.turbo-stream.html`, with no follow-up page GET or CSS/JS re-fetch — before the fix, the same click produced a 302 redirect followed by a full page reload (re-fetching Bootstrap/icons from the CDN every time). 405 tests green (2 new: turbo-stream toggle response test). `rubocop`/`zeitwerk:check` clean.

### Added — Ad Hoc News (ad-hoc-news.de) feeds as a third feed source (no migrations)

- New `Feed::SOURCES` entry `"adhocnews"` alongside `bbc`/`nyt`; `Feed::ADHOCNEWS_FEEDS` is the German-language catalog of all 12 feeds listed at https://www.ad-hoc-news.de/rss (All News, Ad Hoc Notices, Stock Market, Economy, Politics, Sports, Entertainment, Science, Company News, Police Reports, International, Corporate News), mapped onto the existing canonical categories where a good fit exists (`business`, `politics`, `sports`, `arts`, `science`, `world`) plus one new `"police"` category (same precedent as NYT's extra topical sections that also aren't in the nav menu). `Feed.seed_adhocnews_feeds!` follows the same idempotent `find_or_create_by!`-on-`url` pattern as the BBC/NYT seeders; wired into `db/seeds.rb`.
- New `AdhocnewsFeedFetcher < FeedFetcher` (`allowed_hosts: %w[www.ad-hoc-news.de]`) — the site serves Atom (not RSS 2.0), which Feedjira already auto-detects and parses through the same `title`/`url`/`summary`/`published` interface, so no base-class changes were needed. `FeedIngestor::FETCHER_CLASSES` routes `"adhocnews"` feeds to it.
- `Admin::FeedsController#seed` refactored from an if/else into a `SEED_METHODS` lookup (`bbc`/`nyt`/`adhocnews` → seeder method + label) so adding a 4th source later is a one-line change; `/admin/feeds` gained a "Seed Ad Hoc News Feeds" button. `NewsHelper::SOURCE_INFO` gained an `"adhocnews"` entry (name/domain/url) so the public portal's "Read the original on ad-hoc-news.de" link and structured data work for these articles.
- All 12 feed URLs verified live (200 OK, real Atom content) before adding. No schema/migration changes — `source` was already a validated string column. 404 tests green (10 new: `AdhocnewsFeedFetcherTest`, `Feed`/`FeedIngestor`/`Admin::FeedsController` coverage). `rubocop`/`zeitwerk:check` clean.

### Fixed — Phase 1 pipeline reliability from plan2.md (C-4, H-5, H-8, H-9, H-3, H-12, H-11, M-4)

- **C-4** (duplicate task execution race) — `Task::LLMARKT_JOB_TIMEOUT` (20 min) is now the single source of truth for the llmarkt job timeout (`LlmarktSubmitter#submit_request` reads it instead of a hardcoded `20.minutes`), and `Task::STALE_AFTER` is derived from it plus a 10-minute buffer (was a flat 15 minutes, *below* the llmarkt timeout — a legitimately-running llmarkt job could get reclaimed and executed a second time by the Ollama worker). `Task#requeue!` now clears `external_job_id` so a reclaimed task's stale job can't be mistaken for the new attempt (priority/retry mirroring, webhook routing). `LlmarktSubmitter.handle_callback`/new `.handle_failure` run inside `task.with_lock` and ignore any webhook whose `job_id` doesn't match the task's current `external_job_id` (`Api::LlmCallbacksController` now passes `params[:job_id]` through). `Task#complete!`/`#fail!` are idempotent (`return if completed?`) so a late/duplicate callback can never re-run the chain or clobber a completed result.
- **H-5** (non-atomic `complete!`) — `Task#complete!` now writes the target/article/task status inside one `transaction`, and only *after* that commits does it run the chain (`chain_translate!`/`chain_refine!`/`chain_autopost!`/`notify_admin_bot!`) — each individually rescued, matching the pattern `chain_autopost!` already used. A chain failure can no longer flip an already-completed target to `error` via `Api::TasksController#complete`'s rescue, since any exception before the transaction commits now rolls back cleanly instead of leaving a completed target with a task the controller thinks needs `fail!`.
- **H-8** (Telegram Markdown breakage) — `TelegramPoster` and `TelegramAdminNotifier` switched from `parse_mode: "Markdown"` (rejects the whole message on any unbalanced `*`/`_`/`[`/backtick — routine in LLM-generated Persian text) to `parse_mode: "HTML"` with every interpolated value passed through `CGI.escapeHTML`. New test coverage for hostile titles containing `<script>`/`&`/`"`.
- **H-9** (translator not stripping `<think>`) — new shared `LlmText.clean` (strip `<think>…</think>`, `strip`) replaces three near-identical copies of the same regex in `ArticleRewriter`/`TranslationRefiner`; `ArticleTranslator.process` — which never stripped reasoning blocks at all — now uses it too, so a reasoning model (qwen3 family) configured for translation can no longer leak chain-of-thought onto the portal/Telegram.
- **H-3** (no feed-fetch timeout) — `FeedFetcher#entries` now passes `timeout: 10` to `HTTParty.get` (previously no timeout at all, so one slow RSS host could hang the fetch indefinitely).
- **H-12** (task-claim lock contention) — `Task.claim_next!` now locks with `"FOR UPDATE SKIP LOCKED"` (PostgreSQL; a no-op on SQLite, which serializes writers at the connection level anyway) so concurrent worker polls claim different rows instead of queueing behind the head-of-queue lock; `reclaim_stale!` moved outside the claim transaction (it's idempotent and was needlessly lengthening the lock hold).
- **H-11** (`async` Action Cable adapter in production) — added the `solid_cable` gem; production now uses `adapter: solid_cable` in `config/cable.yml`, folded into the primary database (no `connects_to`/separate `cable` db — same pattern as Solid Cache) via a new `solid_cable_messages` table. The previous `async` adapter only delivered broadcasts within the same process (silently dropping anything broadcast from a rake task/console) and would break outright with more than one web process.
- **M-4** (duplicate Telegram posts) — new `Publisher` service (`app/services/publisher.rb`) consolidates the three near-identical create-post/send/update sequences previously duplicated in `Autoposter`, `Admin::TranslationsController#post_to_channel`, and `TelegramAdminNotifier#post_to_channel`; a new unique index on `telegram_posts(translation_id, telegram_channel_id)` (+ matching `TelegramPost` model validation) backs its claim-then-post pattern so the autopost sweep, the task chain, and the admin bot can't race into duplicate posts to the same channel.
- Migrations added for the record (per project rule, **not run** against dev/prod — `db/schema.rb` updated manually and `db:test:prepare` run so tests pass): `20260716000001_add_unique_index_to_telegram_posts.rb`, `20260716000002_create_solid_cable_messages.rb`. **NOTE: dev/prod DBs need `bin/rails db:migrate` before Solid Cable or the new unique index take effect** — if any production `telegram_posts` rows already have duplicate `(translation_id, telegram_channel_id)` pairs, that migration will need those deduped first.
- 395 tests green (2 telegram_poster_test.rb/telegram_admin_notifier_test.rb updated for the HTML parse-mode switch, 1 new HTML-escaping test added, 1 stale tautological test removed that asserted against a hand-built string instead of real service output). `rubocop`/`zeitwerk:check` clean. Brakeman unchanged (still the 1 known M-9 warning). Verified live via `bin/rails runner`: `complete!` transactionality/idempotency/think-stripping and `requeue!` clearing `external_job_id`, plus Solid Cable's `config/cable.yml` resolving correctly.
- **Not done in this pass** (remaining Phase 1 exit-criteria item, and everything in Phases 2–4 — tracked in [plan2.md](plan2.md)): no chaos test was run (kill the worker mid-task, let llmarkt time out, replay duplicate webhooks) — the fixes above were verified individually (unit tests + a scripted `bin/rails runner` smoke check), not under an actual induced race.
...
- The plan catalogs **6 Critical**, **13 High**, **15 Medium**, and 10 low-priority findings with file/line references — headline items: deactivated users keep working sessions (`current_user` never checks `active`), no `reset_session` on login (session fixation), a duplicate-task-execution race between the 15-minute stale-task reclaimer and 20-minute llmarkt jobs, `protect_from_forgery with: :null_session` left on `Admin::TranslationsController`, disabled production host authorization, synchronous third-party geolocation/og:image HTTP calls in the public request path, missing DB indexes, and Telegram Markdown-escaping failures.
- Also includes: refactor proposals (task orchestrator, StoryPool query object, single Telegram `Publisher`, shared LLM-text cleaner), an architecture assessment (cache-as-database anti-pattern, need for an in-app background executor for non-LLM I/O), 15 prioritized feature proposals with effort estimates (portal RSS feeds, pipeline health alerting, persisted tags + tag pages, full-text search, LLM quality review, …), a metrics/validation protocol, and a 5-phase roadmap.

### Changed — Telegram admin bot: skip the channel picker (but keep a one-tap confirm) when only one channel is enabled

- The admin bot's "📤 انتشار در کانال تلگرام" (publish to a Telegram channel) button always opened a channel-picker submenu — pointless friction when the site has just one channel to post to. `TelegramAdminNotifier#publish_button` now, with **exactly one** enabled `TelegramChannel`, renders a direct one-tap post button ("📤 انتشار در <channel name>", callback `post:<id>:<channel_id>`) instead of the submenu button. It does **not** auto-publish — the admin still taps once to send, preserving the human review gate before anything hits the channel; a re-sent notification for an already-posted story shows "✅ ارسال شد به <channel name>" and re-posts if tapped. With **zero or several** enabled channels, behavior is unchanged: the button opens the picker submenu (`channels:<id>`).
- (Supersedes an earlier auto-publish-on-notify approach from this session, which the user reversed — publishing should not happen without an explicit tap.)
- 394 tests green (`telegram_admin_notifier_test.rb`: one enabled channel → direct one-tap button, nothing posted on notify; several enabled channels → submenu, nothing posted on notify). `zeitwerk:check` clean. No migrations.
...
- **Admin/editor prompt editing, not admin-only**: new `/admin/prompts` (`Admin::PromptsController`, index/show/edit/update + `revert`) — deliberately not gated behind `require_admin!`, since editors are expected to tune prompt wording as part of the editorial workflow. The show page lists full version history (who wrote each version, when, a `change_note`, and an expandable "view this version's text" panel) with a "Revert to this version" button on every non-current version. New sidebar link.
- **Task ↔ prompt version traceability**: every request built by the five services now embeds a `prompt_version_id` alongside its existing `key`; `Task#record_prompt_version_usages` (an `after_create` callback) turns that into `PromptVersionUsage` rows (`task`, `prompt_version`, `request_key`). `Rewrite#generating_task`/`Translation#generating_task` (new `has_many :tasks, as: :target` reverse of the existing polymorphic `Task#target`) find the task that produced them; a new shared `admin/shared/_prompt_usages` partial (rendered on the Rewrite/Translation/Task admin show pages) lists "`body` → Rewrite — Body (v3)" etc., linking to the current prompt page. **Since `Task.enqueue_*` always calls `Prompt.current_version`, every newly generated task automatically uses the latest prompt version** — no code changes needed elsewhere to pick that up.
- New `prompts`/`prompt_versions`/`prompt_version_usages` tables (migration `20260705000003`). Schema updated manually per project rules (`db/schema.rb` bumped to `2026_07_05_000003`, `bin/rails db:test:prepare` run — not `db:migrate`); production DB still needs `bin/rails db:migrate` and then `bin/rails db:seed` (or `Prompt.seed_defaults!` via `bin/rails runner`) to populate the initial prompt content — until then, `Task.enqueue_*`/`ArticleRewriter.requests` etc. raise `ActiveRecord::RecordNotFound`.
- 382 tests green (12 new: `PromptTest`, `Admin::PromptsControllerTest`, plus a `TaskTest` case asserting `enqueue_rewrite` links its two requests to the current `rewrite_body`/`rewrite_title` prompt versions). `bin/rails zeitwerk:check` clean.

### Added — Telegram admin notification bot with a 6-option inline-button workflow (migration `20260705000002`)

- **New Telegram bot, separate from the per-channel publishing bots** (`TelegramChannel`/`TelegramPoster`): whenever a translate or refine `Task` completes and its translation becomes the active version — i.e. news ready for a human decision — `Task#complete!` calls the new `TelegramAdminNotifier.notify`, which DMs a configured admin/editor chat with the Persian title/body preview and an inline keyboard offering all 6 requested actions: **request rewrite**, **request retranslation**, **request refine**, **publish to a Telegram channel** (opens a channel-picker submenu), **publish/unpublish on the news portal**, and **mark for manual edit by editors**. New `TelegramAdminBot` config module (credentials-first-then-ENV, mirrors `Llmarkt`/`MailerConfig`: `telegram_admin_bot_token`/`telegram_admin_chat_id`/`telegram_admin_webhook_secret`); no-ops (no message sent) when unconfigured.
- **Inbound webhook**: `POST /api/telegram_admin/webhook` (`Api::TelegramAdminController`) receives Telegram's `callback_query` button taps, authenticated by the `X-Telegram-Bot-Api-Secret-Token` header (constant-time compare against `TelegramAdminBot.webhook_secret`) rather than the worker bearer token or a signed URL token, since Telegram itself is the caller. `TelegramAdminNotifier.handle_callback` (mirrors the existing `LlmarktSubmitter` pattern of one service owning both the outbound call and its matching inbound callback) dispatches the tapped action, reusing the exact same logic as the equivalent admin-UI controller actions (`OllamaServer.pick` + `Task.enqueue_rewrite`/`enqueue_translate`/`enqueue_refine`, `TelegramPoster`/`TelegramPost`, `Translation#archive!`/`unarchive!`), then edits the Telegram message in place (new state + a confirmation banner via `answerCallbackQuery`) and records a `TelegramAdminNotification` row (chat id, message id, last action, who actioned it, when) for audit. `bin/rails telegram_admin:set_webhook`/`:delete_webhook` rake tasks register/unregister the webhook URL with Telegram (`secret_token:` included).
- **New `needs_manual_edit` flag on `Translation`** backs the "mark for manual edit" action end-to-end, not just as a one-way Telegram button: `/admin/translations` shows a "Needs edit" badge + filter toggle, and the translation show page shows a banner with a "Clear flag" button (new `POST .../toggle_manual_edit` route/action) — so a flag set from Telegram is visible and reversible from the web admin, and vice versa.
- New `telegram_admin_notifications` table (translation, chat_id, message_id, status, last_action, actioned_by, actioned_at). Schema updated manually per project rules (`db/schema.rb` bumped to `2026_07_05_000002`, `bin/rails db:test:prepare` run — not `db:migrate`); dev/prod DBs still need `bin/rails db:migrate`.
- 370 tests green (25 new: `TelegramAdminBotTest`, `TelegramAdminNotifierTest`, `Api::TelegramAdminControllerTest`, plus `Translation`/`Task` coverage for the manual-edit flag and the admin-bot notify hook). `bin/rails zeitwerk:check` clean; `rubocop` clean on all new/changed files. Smoke-tested live against a disposable copy of the test DB (real server, real login, real HTTP requests): `/admin/translations` renders the "Needs edit" badge and filter, the translation show page renders the manual-edit banner + "Clear flag" button, and `POST .../toggle_manual_edit` correctly flips the flag and re-renders. The Telegram side (send/receive against the real Bot API) was verified via stubbed `Telegram::Bot::Client` unit/controller tests only, following this codebase's existing `TelegramPosterTest` convention — not exercised against a live bot token in this session.

### Fixed — llmarkt job submission was silently broken (every task falling back to the local Ollama worker)

- Cross-checked our integration against the live spec at `https://www.llmondemand.com/api-docs/v1/swagger.yaml` (same API contract as `llmarkt.codehospital.com`, just the vendor's own docs domain). Found that the `priority:`/`timeout_seconds:` keyword arguments added to the `LlmarktClient.submit_job` call in `LlmarktSubmitter#submit_request` (previous commit) were never added to `LlmarktClient.submit_job`'s own method signature, which had no `priority:`/`timeout_seconds:` keywords and no `**kwargs` catch-all. Every real call raised `ArgumentError: unknown keywords: :timeout_seconds, :priority`, silently rescued by `submit_task`'s `rescue StandardError`, rolling the task back to `pending` — so **every task since that commit was falling through to the Ollama worker fallback instead of running on llmarkt**, and (since no task ever got an `external_job_id`) priority changes/retries were never mirrored to llmarkt either, even though that sync code (`Task#reprioritize!` → `LlmarktSubmitter.update_priority` → `PATCH /jobs/:id/priority`) was itself correct.
- `LlmarktClient.submit_job` now accepts optional `priority:`/`timeout_seconds:` keywords and includes them in the `POST /jobs` body when present, matching the documented `JobCreateRequest` schema.
- The test suite didn't catch this because `LlmarktSubmitterTest` stubbed `LlmarktClient.submit_job` with `->(**) { ... }`, which swallows any keyword arguments regardless of the real method's signature. Added `LlmarktSubmitterTest#"submit_task posts priority and timeout_seconds through the real submit_job signature"`, which stubs only the HTTP layer (via WebMock) and lets the real `submit_job` method run — this is the test that would have caught the mismatch. Also added direct `LlmarktClientTest` coverage for `priority`/`timeout_seconds` passthrough (present and absent).
- Also found and fixed a related mismatch: the vendor's current docs name the webhook signature header `X-LLMOnDemand-Signature`, but `Api::LlmCallbacksController` only checked the older `X-Vibe-Signature` name. Now checks `X-LLMOnDemand-Signature` first, falling back to `X-Vibe-Signature` for compatibility, so a callback is accepted either way. 345 tests green (4 new).

### Added — Admin bootstrap credentials + Resend sender email movable into Rails credentials

- New `AdminBootstrap` service module (`app/services/admin_bootstrap.rb`) mirrors the `Llmarkt`/`MailerConfig` credentials-first-then-ENV pattern for the one-time bootstrap admin: `credentials.admin_username`/`admin_password`/`admin_email`, falling back to `ADMIN_USERNAME`/`ADMIN_PASSWORD`/`ADMIN_EMAIL`. `db/seeds.rb` now reads through it instead of `ENV` directly. `config/initializers/required_env.rb`'s boot-time presence check now accepts either source too — kept deliberately self-contained (no reference to the autoloaded `AdminBootstrap` constant) since `config/initializers/*.rb` files run before the app's autoloader is set up (see the Fixed entry below).
- `MailerConfig.from_address` now reads `credentials.sender_email` / `ENV["SENDER_EMAIL"]` (renamed from `mailer_from_address`/`MAILER_FROM_ADDRESS`) to match the key name actually used in this app's credentials. `.env.example`/README updated to match.
- New `AdminBootstrapTest` (credentials-over-ENV precedence, ENV fallback, `configured?`). Verified live via `bin/rails runner` in development that the app boots and fully resolves `AdminBootstrap`/`MailerConfig`/`ActionMailer::Base` settings from Rails credentials alone, with no ENV vars set at all. 341 tests green (3 new).

### Fixed — `NameError: uninitialized constant MailerConfig` on boot (`rake db:migrate`, `bin/rails server`, etc.)

- `config/initializers/action_mailer.rb` referenced `MailerConfig`/`Llmarkt` (autoloaded `app/services/` constants) directly at the top level of the initializer. At that point in Rails' boot sequence the main Zeitwerk autoloader isn't set up yet, so any autoloaded app constant raises `NameError` — confirmed this is a general boot-time gotcha (not specific to the new file) by reproducing the identical failure with the pre-existing `Llmarkt` constant referenced the same way. Fixed by wrapping the whole initializer body in `Rails.application.config.to_prepare { ... }`, which runs once after the autoloader is fully active (and again on each reload in development); verified via `bin/rails runner` that `ActionMailer::Base.perform_deliveries`/`.smtp_settings`/`.default_url_options` all still end up set correctly. 338 tests still green.

### Added — User emails + "forgot password" via Resend (migration `20260705000001`)

- **`users.email`** column (unique, required, lowercase-normalized, format-validated). `db/seeds.rb`'s bootstrap admin now also needs `ADMIN_EMAIL` (added to `config/initializers/required_env.rb`'s required list alongside `ADMIN_USERNAME`/`ADMIN_PASSWORD`); `/admin/users` create/edit forms gained an email field.
- **Password reset via Rails' built-in `generates_token_for`** (no separate tokens table): `User` calls `generates_token_for :password_reset, expires_in: 20.minutes do password_salt&.last(10) end` — folding `password_salt` (from `has_secure_password`) into the signature means a token is automatically invalidated the moment the password changes, including by using the token itself. New public (unauthenticated) `Admin::PasswordResetsController` (`layout false`, like the login page) — `new`/`create` for the "forgot password?" email-entry form, `edit`/`update` (looked up via `User.find_by_token_for`) for setting a new password. `create` always redirects with the same message regardless of whether the email matched an account, so it can't be used to enumerate registered users. "Forgot password?" link added under the login form.
- **`UserMailer#password_reset`** (new `app/mailers/`, HTML + text views, `layouts/mailer`) emails the reset link (`edit_admin_password_reset_url`).
- **Resend SMTP relay for outgoing mail** — enabled `action_mailer/railtie` (was commented out; no mail had been sent from this app before). New `MailerConfig` service module mirrors `Llmarkt`'s credentials-first-then-ENV pattern (`resend_api_key`/`RESEND_API_KEY`, `mailer_from_address`/`MAILER_FROM_ADDRESS`); `config/initializers/action_mailer.rb` sets `smtp_settings` (`smtp.resend.com:587`, username `"resend"`, password = the API key) and `default_url_options` (host/port/protocol parsed from `Llmarkt.app_base_url`) only when configured — `perform_deliveries` stays off otherwise, so the reset flow still runs end-to-end without erroring when Resend isn't set up yet. `config/environments/test.rb` uses the `:test` delivery method (asserted via `ActionMailer::Base.deliveries`/`assert_emails`); `development.rb` gets a `localhost:3000` `default_url_options` fallback. `bin/docker-entrypoint` now also runs `bin/rails db:seed` on boot (idempotent) so the bootstrap admin actually gets created on a fresh deploy.
- Tests: new `UserMailerTest`, `Admin::PasswordResetsControllerTest` (send/no-send for known vs. unknown vs. disabled email, valid/invalid/expired-by-use token, mismatched confirmation), `User` email validation tests; `test/test_helper.rb` factories and other direct `User.create!` calls updated with unique emails. 338 tests green (15 new).
- NOTE: dev/prod DBs still need `bin/rails db:migrate` (not run here per project rules) for the `email` column, then `ADMIN_EMAIL` must be set before `bin/rails db:seed`/first boot.

### Added — Editor accounts, roles, and a "who did what" audit trail (migrations `20260704000001`, `20260704000002`)

- **`User` model + multi-account login**: replaces the single `ADMIN_USERNAME`/`ADMIN_PASSWORD` env-var login with a real `users` table (`has_secure_password`, `role` — `admin`/`editor`, `active`). `Admin::SessionsController` now authenticates against `User.authenticate` and stores `session[:user_id]`; `ApplicationController#current_user` resolves it. `ADMIN_USERNAME`/`ADMIN_PASSWORD` are now only a one-time bootstrap: `db/seeds.rb` creates the first admin from them when the `users` table is empty (also wired into the Docker entrypoint's boot sequence, `bin/rails db:seed` — idempotent). `ApplicationCable::Connection` and its test updated to the same `session[:user_id]` check.
- **Roles**: `admin` keeps full access; `editor` is redirected (with an alert) away from infrastructure/config pages — Feeds, Telegram Channels, Ollama Servers, House Keeping, IP Geolocations, Users, and the new Activity Log — via a shared `Admin::BaseController#require_admin!` before_action. Editors keep the full editorial workflow: Dashboard, Articles, Rewrites, Translations, Task queue, Analytics, and the (read-only) Telegram Posts log. The admin sidebar and dashboard stat cards hide admin-only links/cards for editors.
- **`/admin/users`** (admin-only): list/create/edit accounts, set role and display name, reset a password (blank = keep current), and enable/disable via a toggle button (same pattern as the existing Feed/Channel toggles). A model validation (`User#at_least_one_active_admin_remains`) blocks demoting or disabling the last active admin, so nobody can lock everyone (including themselves) out.
- **"Who did what" + full version history via [PaperTrail](https://github.com/paper-trail-gem/paper_trail)** (new `bcrypt` + `paper_trail` gems, new `versions` table): `has_paper_trail` added to `Article`, `Rewrite`, `Translation`, `Feed`, `TelegramChannel` (excluding the bot `token` from snapshots), `OllamaServer`, and `User` (excluding `password_digest`). `ApplicationController` sets `PaperTrail.request.whodunnit` to `current_user.id` on every request (nil/"AI worker / system" for background changes made outside a request, e.g. a worker completing a Task). Rewrite/Translation show pages gain an **Edit history** panel (`admin/rewrites/_history`, `admin/translations/_history`) listing every past version with who made it and the prior text (via `version.reify`). New `/admin/activity_logs` (admin-only) is a system-wide, paginated audit log across every tracked model, filterable by model type or user, with links to the current record (or "(deleted)" when it's gone). Shared `VersionsHelper` (`version_actor`, `version_record_path`) backs both views.
- `config/application.rb` sets `config.active_record.yaml_column_permitted_classes` (adds `ActiveSupport::TimeWithZone`/`ActiveSupport::TimeZone` to Rails' default) — required for `version.reify` to deserialize timestamp attributes from the YAML-serialized `versions.object` column under Rails' safe YAML loader.
- Test suite: `test/test_helper.rb` gains `create_admin_user`/`create_editor_user`/`log_in_as` helpers (replacing the old ENV-var + hardcoded-credential `log_in` duplicated in every controller test); all existing admin controller tests migrated to it. New `UserTest`, `Admin::UsersControllerTest`, `Admin::ActivityLogsControllerTest`, plus editor-forbidden-page tests on Feeds/Housekeeping and edit-history tests on Rewrites/Translations. 323 tests green (29 new).
- NOTE: dev/prod DBs still need `bin/rails db:migrate` (not run here per project rules) to create the `users`/`versions` tables, then `bin/rails db:seed` to bootstrap the first admin account from `ADMIN_USERNAME`/`ADMIN_PASSWORD`.
